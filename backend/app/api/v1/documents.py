...
async def upload_document(
    ...
    if not member or member.role != "owner":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="权限不足，只有组织创建者可以上传文档")
...
async def delete_documents_batch(
    ...
    if not member or member.role != "owner":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="权限不足，只有组织创建者可以删除文档")

    deleted_count, cleanup_list = await document_service.delete_documents_batch(db, body.ids, str(current_user.id), org_id)
    if cleanup_list:
        document_service.cleanup_documents_batch(cleanup_list, org_id=org_id)
...
async def get_document(
    ...
    document = await document_service.get_document(db, document_id, str(current_user.id), org_id)
    if not document:
...
async def delete_document(
    ...
    if not success:
        raise HTTPException(status_code=404, detail="文档不存在")
    if filename:
        document_service.cleanup_document_resources(document_id, filename, org_id=org_id)
...
async def get_document_chunks(
    ...
    chunks = await document_service.get_chunks(db, document_id, str(current_user.id), org_id)
    chunk_infos = [
        ChunkInfo(
            id=c.get("id", ""),
...
async def search_documents(
    ...
    results = await document_service.search(body.query, body.top_k, org_id=org_id)
